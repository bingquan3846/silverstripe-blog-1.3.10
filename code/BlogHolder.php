...
class BlogHolder extends BlogTree implements PermissionProvider {
	static $icon = "sapphire/javascript/tree/images/page-openfolder";
	...
	function getCMSFields() {
		...
		$ownerDropdown = new DropdownField('OwnerID', 'Blog owner', $blogOwners->map('ID', 'Name'));
        $ownerDropdown->setEmptyString('(None)');
        $ownerDropdown->setHasEmptyDefault(true);
        ...
        $ownerCheckbox = new CheckboxField("ShowFullEntry", "Show Full Entry");
        //$ownerCheckbox->setDescription('Show full content in overviews rather than summary');
		$fields->addFieldToTab(
			"Root.Main",
            $ownerCheckbox
		);
	...
	function blogOwners($sort = 'FirstName', $direction = null) {
